...
import serializers.thrift.media.Pod;
...
public class Thrift
{
	...
	public static final MediaTransformer<MediaContent> mediaTransformer;

	static {
		mediaTransformer = new MediaTransformer<MediaContent>() {
			@Override
			...
			private Media forwardMedia(data.media.Media media) {
				...
				for (data.media.Pod pod : media.pods) {
					mb.addToPods(forwardPod(pod));
				}
			...
			private Pod forwardPod(data.media.Pod pod) {
				Pod p = new Pod();
				p.setMessage(pod.getMessage());
				if (pod.getPod() != null) {
					p.setPod(forwardPod(pod.getPod()));
				}
				return p;
			}
